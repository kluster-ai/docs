...
Developers can access the reliability check feature via the regular chat completion endpoint to validate responses in complete conversation histories using the same format as the standard chat completions API. This approach enables verification of reliability within the entire context of a conversation.
...
This example demonstrates how to utilize the service with the chat completion endpoint via the OpenAI `/v1/chat/completions` endpoint and OpenAI libraries, employing the specialized `klusterai/verify-reliability` model to enable a Verify reliability check.
...
- Review the complete [API documentation](/api-reference/reference/#/http/api-endpoints/realtime/v1-verify-reliability-post){target=\_blank} for detailed specifications of each endpoint.
