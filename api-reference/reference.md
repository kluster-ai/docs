...
`POST https://api.kluster.ai/v1/verify/reliability`
...
The original question or instruction.

---

`output` ++"string"++ <span class="required" markdown>++"required"++</span>

The LLM response you want to verify.

---

`context` ++"string or null"++

Optional reference material. When present, reliability is checked only against this context.

---

`return_search_results` ++"boolean or null"++

Whether to return the web search snippets the judge used to fact-check the answer. Defaults to `false`.

---

**Returns**

A [Reliability response object](#reliability-response-object) indicating if a hallucination was detected.
...
=== "curl"
    ...
    ```bash title="Example request"
    curl -s https://api.kluster.ai/v1/verify/reliability \
      -H "Authorization: Bearer INSERT_API_KEY" \
      -H "Content-Type: application/json" \
      -d '{
            "prompt": "Is the Earth flat?",
...
```json title="Response"
{
  ...
  "explanation": "The response 'Yes, absolutely' is factually incorrect as it contradicts the established scientific fact that the Earth is not flat.\nThe statement is an unsupported claim because it goes against the overwhelming scientific consensus and evidence.\nThe response confidently presents a falsehood, which is a key characteristic of hallucination.",
  "search_results": []
}
```
...
Returns `true` if the answer contains unreliable or unsupported information.

---

`explanation` ++"string"++

Reasoning behind the judgment. Useful for audit logs and model evaluation.

---

`usage` ++"object"++

Token-level usage statistics for billing / monitoring.
...
??? child "Show properties"
    ...
    Tokens generated during the verification step.
...
Only present when `return_search_results=true`. Each element contains:

| Field | Type   | Description                           |
|-------|--------|---------------------------------------|
| `title`   | string | Page title                            |
| `snippet` | string | Passage supporting/refuting the claim |
| `link`    | string | Source URL                            |

</div>
<div markdown>

```json title="Reliability response object"
{
  "is_hallucination": false,
  "explanation": "Answer matches the supplied context exactly.",
  "usage": {
    "prompt_tokens": 58,
  ...
  "search_results": [
    {
      "title": "NASA – Shape of the Earth",
      "snippet": "...the planet is an oblate spheroid...",
