...
4. Under **Scopes**, select **Full access**, or **Custom defined** and enable the following permissions:

    |  Category   |     Scope      |               Description               |
    |:-----------:|:--------------:|:---------------------------------------:|
    |    Code     |    **Read**    |       Read source code and metadata.       |
    |    Code     |   **Write**    |   Create and update repository content.    |
    | Code        |  **Status**    |   Read and write commit status.            |
    | Pull Request Threads | **Read & write** | Read and create pull request comments. |

    !!! note
        The exact scopes required may vary. Ensure the token has sufficient permissions to read repository content, read pull requests, and post comments on pull requests.
...
- A **summary comment** titled **kluster.ai PR Review Summary**, which includes a description of the changes, the review result (All Clear or a list of detected issues), and a prior review warning if no IDE or CLI reviews were performed on the branch.
    ...
    <!-- TODO: add proper image - Example of a kluster.ai PR Review Summary comment on an Azure DevOps pull request -->
